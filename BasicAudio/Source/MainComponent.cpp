...
public:
    ...
    { 
        ...
        frequencySlider[i].setRange (50, 2000);
        ...
        frequencyAllSlider.setValue(261.25); // will also set the default frequency of the sine osc
        ...
        for (int sample = 0; sample < bufferToFill.numSamples; ++sample)
        {
            if(onOffAll == 1) {
                ...
                for (int i = 0; i < Oscs; i++){      
                    ...
                    if(onOff[i]) {
                        temp += onOff[i]* sine[i].tick() * 0.5 * gain[i];
                    }
                }
                buffer[sample] =temp;
            }
